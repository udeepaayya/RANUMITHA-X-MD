...
cmd({
  ...
  try {
    ...
    const res = await axios.get(`https://lance-frank-asta.onrender.com/api/downloader?url=${q}`);
    const data = res.data?.content?.data;

    if (!data) return reply("⚠️ Failed to retrieve video details. Try again.");

    const high = data.result?.find(v => v.quality?.toUpperCase() === "HD")?.url;
    const low = data.result?.find(v => v.quality?.toUpperCase() === "SD")?.url;
      ...
      if (isReplyToBot) {
        ...
        try {
          switch (text) {
            case "1":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { video: { url: low }, caption: "📥 *Downloaded in SD Quality*" }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            case "2":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { video: { url: high }, caption: "📥 *Downloaded in HD Quality*" }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            case "3":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { audio: { url: low || high }, mimetype: "audio/mp4", ptt: false }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            default:
...
cmd({
  ...
  try {
    ...
    if (!data) return reply("⚠️ Could not fetch video data.");

    const hdVideo = data.result?.find(v => v.quality?.toUpperCase() === "HD")?.url;
    const sdVideo = data.result?.find(v => v.quality?.toUpperCase() === "SD")?.url;
    const thumbnail = data.thumbnail || data.thumb || data.image;
      ...
      if (isReplyToBot) {
        ...
        try {
          switch (text) {
            case "1":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { video: { url: sdVideo }, caption: "📥 *Downloaded in SD Quality*" }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            case "2":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { video: { url: hdVideo }, caption: "📥 *Downloaded in HD Quality*" }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            case "3":
              await conn.sendMessage(senderID, { react: { text: '⬆️', key: receivedMsg.key } });
              await conn.sendMessage(senderID, { audio: { url: sdVideo || hdVideo }, mimetype: "audio/mp4", ptt: false }, { quoted: receivedMsg });
              await conn.sendMessage(senderID, { react: { text: '✅', key: receivedMsg.key } });
              break;
            default:
          ...
          reply("⚠️ Failed to send file. Try again.");
        }
      }
    });
