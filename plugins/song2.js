...
cmd({
  ...
  alias: ["play2", "song2"],
  react: "🎵",
  desc: "Download YouTube song (Audio) via Nekolabs API (with Shorts support, single reply)",
  category: "download",
...
> © Powerd by 𝗥𝗔𝗡𝗨𝗠𝗜𝗧𝗛𝗔-𝗫-𝗠𝗗 🌛`;
    ...
    // 🟢 Listen for one-time user reply
    const handler = async (msgUpdate) => {
      try {
        ...
        if (!isReply || !["1", "2", "3"].includes(choice)) return;
        ...
        // ✅ Choice 1: Audio
        if (choice === "1") {
          type = { audio: { url: dlUrl }, mimetype: "audio/mpeg", fileName: audioFileName };

        // ✅ Choice 2: Document
        } else if (choice === "2") {
          type = { document: { url: dlUrl }, mimetype: "audio/mpeg", fileName: audioFileName, caption: meta.title };
        ...
        // ✅ Choice 3: Voice note (convert to .opus)
          ...
          type = { audio: voiceBuffer, mimetype: "audio/ogg; codecs=opus", ptt: true };
