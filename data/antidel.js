...
const AntiDelDB = DATABASE.define('AntiDelete', {
    ...
    status: {
        ...
        defaultValue: config.ANTI_DELETE || true, // 🔹 Default TRUE
    },
}, {
    ...
    try {
        ...
        if (tableInfo.gc_status) {
            ...
            await AntiDelDB.findOrCreate({
                where: { id: 1 },
                defaults: { status: config.ANTI_DELETE || true }, // 🔹 Default TRUE
            });
        }
        isInitialized = true;
    } catch (error) {
        ...
        if (error.original && error.original.code === 'SQLITE_ERROR' && error.original.message.includes('no such table')) {
            await AntiDelDB.sync();
            await AntiDelDB.create({ id: 1, status: config.ANTI_DELETE || true }); // 🔹 Default TRUE
            isInitialized = true;
        }
    }
}
...
async function getAnti() {
    try {
        ...
        return record ? record.status : (config.ANTI_DELETE || true); // 🔹 Default TRUE
    } catch (error) {
        console.error('Error getting anti-delete status:', error);
        return config.ANTI_DELETE || true; // 🔹 Default TRUE
    }
}
